...
fn get_catalog(doc: &Document) -> Result<&Dictionary, OutputError> {
    ...
    Err(OutputError::PdfError(Error::Invalid(format!(
        "catalog not found"
    )))) //panic!();
}
...
fn encoding_to_unicode_table(name: &[u8]) -> Vec<u16> {
    let encoding = match &name[..] {
        ...
        _ => {
            dlog!("unexpected encoding {:?}", pdf_to_utf8(name));
            return Vec::new();
        }
    };
                                    ...
                                    match unicode_map {
                                            ...
                                            if base_name.contains("FontAwesome") =>
                                        {
                                            ...
                                            match unicode_map.entry(code as u32) {
                                                ...
                                                Entry::Occupied(e) => {
                                                    dlog!("unexpected entry in unicode map")
                                                }
                                            }
                                        }
                                        _ => {
...
impl<'a> PdfFont for PdfType3Font<'a> {
    ...
    fn decode_char(&self, char: CharCode) -> String {
        ...
        if let Some(ref unicode_map) = self.unicode_map {
            ...
            let s = match s {
                None => {
                    dlog!("missing char {:?} in map {:?}", char, unicode_map);
                    String::new()
                }
                Some(s) => s.clone(),
            };
...
impl Function {
    fn new(doc: &Document, obj: &Object) -> Result<Self, OutputError> {
        let dict = match obj {
            ...
            _ => {
                return Err(OutputError::PdfError(Error::Invalid(format!(
                    "unexpected dictionary object {:?}",
                    obj
                ))))
            } //panic!()
        };
        ...
        let f = match function_type {
            ...
            _ => {
                dlog!("unhandled function type {}", function_type);
                return Err(OutputError::PdfError(Error::Invalid(format!(
                    ...
                    "Separation" => {
                        ...
                        let alternate_space = match &maybe_deref(doc, &cs[2]) {
                            Object::Name(name) => match &name[..] {
                                ...
                                _ => {
                                    dlog!("unexpected color space name");
                                    return Err(OutputError::PdfError(Error::Invalid(format!(
                                        "unexpected color space name "
                                    ))));
                                }
                            },
                            Object::Array(cs) => {
                                ...
                                match cs_name.as_ref() {
                                    ...
                                    _ => {
                                        dlog!("Unexpected color space name");
                                        return Err(OutputError::PdfError(Error::Invalid(
                                            format!("Unexpected color space name"),
                                        )));
                                    }
                                }
                            }
                            _ => {
                                dlog!("Alternate space should be name or array {:?}", cs[2]);
                                return Err(OutputError::PdfError(Error::Invalid(format!(
...
pub fn print_metadata(doc: &Document) {
    ...
    dlog!(
        "Page count: {}",
        get::<i64>(&doc, &get_pages(&doc).unwrap(), b"Count")
    );
