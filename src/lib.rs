...
impl std::fmt::Display for OutputError {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), std::fmt::Error> {
        match self {
            ...
            OutputError::PageNumberNotFound(page_num) => {
                write!(f, "PDF error: page number not found: {}", page_num)
            }
        }
    }
}
...
fn get_name_string(dict: &Dict, key: &[u8]) -> String {
    let name: Name = dict.get(key).unwrap_or_else(|| panic!("deref"));
    pdf_to_utf8(&name)
}
...
fn get_stream_contents(stream: &Stream) -> Vec<u8> {
    stream
        .decoded()
        .unwrap_or_else(|_| stream.raw_data().into_owned())
}
...
impl<'a> PdfSimpleFont<'a> {
    fn new(font: &Dict<'a>) -> PdfSimpleFont<'a> {
        ...
        match encoding {
            ...
            Some(Object::Dict(ref encoding_dict)) => {
                ...
                let mut table =
                    if let Some(base_encoding) = maybe_get_name(encoding_dict, b"BaseEncoding") {
        ...
        let widths: Option<Vec<f64>> = font
            .get::<Array>(&b"Widths"[..])
            .map(|a| a.iter::<Number>().map(|n| n.as_f64()).collect());
    ...
    fn get_descriptor(&self) -> Option<PdfFontDescriptor<'a>> {
        self.font
            .get::<Dict>(&b"FontDescriptor"[..])
            .map(|desc| PdfFontDescriptor { desc })
    }
}
    ...
    fn new(font: &Dict<'a>) -> PdfType3Font<'a> {
        ...
        match encoding {
            ...
            Some(Object::Dict(ref encoding_dict)) => {
                ...
                let name_encoded = encoding_dict.get::<Name>(&b"Type"[..]);
                if let Some(name) = name_encoded {
        ...
        let first_char: i64 = font
            .get::<Number>(&b"FirstChar"[..])
            .expect("FirstChar")
            .as_i64();
        let last_char: i64 = font
            .get::<Number>(&b"LastChar"[..])
            .expect("LastChar")
            .as_i64();
...
impl<'a> PdfCIDFont<'a> {
    fn new(font: &Dict<'a>) -> PdfCIDFont<'a> {
        let base_name = get_name_string(font, b"BaseFont");
        let descendants: Array = font
            .get::<Array>(&b"DescendantFonts"[..])
            .expect("Descendant fonts required");
        let ciddict: Dict = descendants
            .iter::<Dict>()
            .next()
            .expect("should have CID dict");
        let encoding: Object = font
            .get::<Object>(&b"Encoding"[..])
            .expect("Encoding required in type0 fonts");
        ...
        let default_width = ciddict
            ...
            .map(|n| n.as_i64())
            .unwrap_or(1000);
        let w: Option<Vec<Object>> = ciddict
            .get::<Array>(&b"W"[..])
            .map(|w| w.iter::<Object>().collect());
        dlog!("widths {:?}", w);
        ...
        if let Some(w) = w {
            while i < w.len() {
                if let Object::Array(ref wa) = w[i + 1] {
                    ...
                    let mut j = 0;
                    dlog!("wa: {:?} -> {:?}", cid, wa);
                    for w in wa.iter::<Number>() {
                        widths.insert((cid + j) as CharCode, w.as_f64());
                        j += 1;
                    }
                    i += 2;
                } else {
                    let c_first = match &w[i] {
                        Object::Number(n) => n.as_i64(),
                        _ => panic!("first should be num"),
                    };
                    let c_last = match &w[i + 1] {
                        Object::Number(n) => n.as_i64(),
                        _ => panic!("last should be num"),
                    };
                    let c_width = match &w[i + 2] {
...
impl Function {
    fn new(obj: &Object) -> Function {
        let dict = match obj {
            Object::Dict(ref dict) => dict,
            Object::Stream(ref stream) => &stream.dict(),
            _ => panic!(),
        };
        let function_type: i64 = dict
            .get::<Number>(&b"FunctionType"[..])
            .expect("FunctionType")
            .as_i64();
        let f = match function_type {
            0 => {
                ...
                let range: Vec<f64> = dict
                    .get::<Array>(&b"Range"[..])
                    .expect("Range")
                    .iter::<Number>()
                    .map(|n| n.as_f64())
                    .collect();
                let domain: Vec<f64> = dict
                    .get::<Array>(&b"Domain"[..])
                    .expect("Domain")
                    .iter::<Number>()
                    .map(|n| n.as_f64())
                ...
                let bits_per_sample = dict
                    .get::<Number>(&b"BitsPerSample"[..])
                    .expect("BitsPerSample")
                    .as_i64();
                // We ignore 'Order' like pdfium, poppler and pdf.js

                let encode: Option<Vec<f64>> = dict
                    .get::<Array>(&b"Encode"[..])
                    .map(|a| a.iter::<Number>().map(|n| n.as_f64()).collect());
            ...
            2 => {
                ...
                let c0: Option<Vec<f64>> = dict
                    .get::<Array>(&b"C0"[..])
                    .map(|a| a.iter::<Number>().map(|n| n.as_f64()).collect());
                let c1: Option<Vec<f64>> = dict
                    .get::<Array>(&b"C1"[..])
                    .map(|a| a.iter::<Number>().map(|n| n.as_f64()).collect());
...
fn apply_state<'a>(gs: &mut GraphicsState<'a>, state: &Dict<'a>) {
    for (k, v) in state.entries() {
        let v = match v {
            hayro_syntax::object::MaybeRef::Ref(_) => {
                // Indirect reference: resolve via Dict::get which dereferences the object
                match state.get::<Object>(&*k) {
                    Some(obj) => obj,
                    None => continue,
                }
            }
        ...
        Object::Array(ref cs) => {
            ...
            let cs_name = pdf_to_utf8(
                cs_items[0]
                    .clone()
                    .into_name()
                    .expect("first arg must be a name")
                    .as_ref(),
            );
            match cs_name.as_ref() {
                ...
                "CalGray" => {
                    let dict = cs_items[1]
                        .clone()
                        .into_dict()
                        .expect("second arg must be a dict");
...
fn make_colorspace<'a>(name: &[u8], resources: &Resources<'a>) -> ColorSpace {
    match name {
        ...
        _ => {
            ...
            let cs: Object = resources
                .get_color_space(cs_name)
                .unwrap_or_else(|| panic!("missing colorspace {:?}", std::str::from_utf8(name)));
...
impl<'a> Processor<'a> {
    ...
    fn process_stream(
        ...
        for operation in TypedIter::new(content) {
            //dlog!("op: {:?}", operation);
            ...
            match operation {
                ...
                TypedInstruction::ColorSpaceStroke(cs) => {
                    gs.stroke_colorspace = make_colorspace(&cs.0, resources);
                }
                TypedInstruction::ColorSpaceNonStroke(cs) => {
                    gs.fill_colorspace = make_colorspace(&cs.0, resources);
                }
                TypedInstruction::StrokeColor(c) => {
                    gs.stroke_color = match gs.stroke_colorspace {
                        ...
                        _ => c.0.iter().map(|n| n.as_f64()).collect(),
                    };
                }
                ...
                | TypedInstruction::NonStrokeColorCmyk(_) => {
                    dlog!("unhandled color operation");
                }
                TypedInstruction::ShowTexts(texts) => {
                    for e in texts.0.iter::<Object>() {
                        match e {
                            Object::String(ref s) => {
                ...
                TypedInstruction::ShowText(s) => {
                    show_text(&mut gs, &s.0.get(), &tlm, &flip_ctm, output)?;
                }
                TypedInstruction::CharacterSpacing(n) => {
                    gs.ts.character_spacing = n.0.as_f64();
                }
                TypedInstruction::WordSpacing(n) => {
                    gs.ts.word_spacing = n.0.as_f64();
                }
                TypedInstruction::HorizontalScaling(n) => {
                    gs.ts.horizontal_scaling = n.0.as_f64() / 100.;
                }
                TypedInstruction::TextLeading(n) => {
                    gs.ts.leading = n.0.as_f64();
                }
                TypedInstruction::TextFont(tf) => {
                    let name_bytes = tf.0.as_ref().to_vec();
                    let font_name = tf.0.clone();
                    let font = font_table
                        .entry(name_bytes)
                        .or_insert_with(|| {
                            let font_dict: Dict =
                                resources.get_font(font_name).expect("font not found");
                            make_font(&font_dict)
                        })
                        .clone();
                    {
                    ...
                    gs.ts.font_size = tf.1.as_f64();
                    dlog!("font {:?} size: {}", tf.0, gs.ts.font_size);
                }
                TypedInstruction::TextRise(n) => {
                    gs.ts.rise = n.0.as_f64();
                }
                TypedInstruction::SetTextMatrix(tm) => {
                ...
                TypedInstruction::NextLineAndSetLeading(td) => {
                    /* Move to the start of the next line, offset from the start of the current line by (tx , ty ).
                ...
                TypedInstruction::SetGraphicsState(gs_name) => {
                    let state: Dict = resources
                        .get_ext_g_state(gs_name.0)
                        .expect("ExtGState not found");
                ...
                TypedInstruction::LineWidth(n) => {
                    gs.line_width = n.0.as_f64();
                }
                TypedInstruction::LineCap(_)
                | TypedInstruction::LineJoin(_)
                | TypedInstruction::MiterLimit(_)
                | TypedInstruction::DashPattern(_)
                | TypedInstruction::RenderingIntent(_) => {
                    dlog!("unknown graphics state operator {:?}", operation);
                }
                TypedInstruction::MoveTo(op) => {
                    path.ops.push(PathOp::MoveTo(op.0.as_f64(), op.1.as_f64()))
                }
                TypedInstruction::LineTo(op) => {
                    path.ops.push(PathOp::LineTo(op.0.as_f64(), op.1.as_f64()))
                }
                TypedInstruction::CubicTo(op) => path.ops.push(PathOp::CurveTo(
                    op.0.as_f64(),
                    op.1.as_f64(),
                    op.2.as_f64(),
                    op.3.as_f64(),
                ...
                TypedInstruction::CubicStartTo(op) => {
                ...
                TypedInstruction::CubicEndTo(op) => path.ops.push(PathOp::CurveTo(
                    op.0.as_f64(),
                    op.1.as_f64(),
                    op.2.as_f64(),
                    op.3.as_f64(),
                ...
                TypedInstruction::ClosePath(_) => path.ops.push(PathOp::Close),
                TypedInstruction::RectPath(op) => path.ops.push(PathOp::Rect(
                    op.0.as_f64(),
                    op.1.as_f64(),
                    op.2.as_f64(),
                    op.3.as_f64(),
                ...
                TypedInstruction::ClipNonZero(_) | TypedInstruction::ClipEvenOdd(_) => {
...
impl<'a> HTMLOutput<'a> {
    pub fn new(file: &'a mut dyn std::io::Write) -> HTMLOutput<'a> {
        HTMLOutput {
            file,
...
pub fn print_metadata(pdf: &Pdf) {
    ...
    if let Some(ref keywords) = metadata.keywords {
        dlog!("Keywords: {}", pdf_to_utf8(keywords));
    }
...
pub fn extract_text<P: std::convert::AsRef<std::path::Path>>(
    ...
    let mut s = String::new();
    {
        ...
        let data = std::fs::read(path).map_err(OutputError::IoError)?;
...
pub fn output_doc_page(
    ...
    let page = &page_num
        ...
        .ok_or(OutputError::PageNumberNotFound(page_num))?;
    let mut p = Processor::new();
    output_doc_inner(page_num, page, &mut p, output)?;
    Ok(())
}
