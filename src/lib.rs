...
impl<'a> PdfFont for PdfSimpleFont<'a> {
    fn get_width(&self, id: CharCode) -> f64 {
        ...
        if let Some(width) = width {
            ...
            dlog!(
                "missing width for {} len(widths) = {}, {:?} falling back to default_width {:?}",
                id,
                self.widths.len(),
                widths,
                self.font
...
// Parse given page and output it to `output`
pub fn output_page(
    doc: &Document,
    output: &mut dyn OutputDev,
    page_num: &u32,
    ...
    let mut p = Processor::new();
    let target_page = pages.into_iter().find(|p| p.0 == *page_num);
    if target_page.is_none() {
        return Err(OutputError::PdfError(Error::PageNumberNotFound(*page_num)));
    }
    process_page(&doc, target_page.unwrap(), output, empty_resources)?;
    Ok(())
}
...
pub fn output_doc(doc: &Document, output: &mut dyn OutputDev) -> Result<(), OutputError> {
    ...
    let pages = doc.get_pages();
    for dict in pages {
        process_page(&doc, dict, output, empty_resources)?;
    }
    Ok(())
}
